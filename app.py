...
BUCKET = "black-outcomes-intelligence-dev-4623dd8370ffd332f292468c61"
YEAR = 2024
STATE_NAMES = {
...
def load_data():
    ...
    return read(f"published/year={YEAR}/national.csv"), read(f"published/year={YEAR}/states.csv")
...
try:
    ...
    st.error("Unable to load the published analytics dataset. Configure AWS credentials with read access to the project S3 bucket.")
    st.exception(exc)
    st.stop()
...
metric_map = {
    "Black population":"black_population",
    "Marriage rate":"married_rate",
    "Bachelor's+ rate":"bachelors_plus_rate",
    "Mean personal income":"weighted_mean_personal_income",
}
column = metric_map[metric]
ranked = states.sort_values(column, ascending=False).copy()
chart = ranked.set_index("state_name")[[column]].head(15)
st.bar_chart(chart)
